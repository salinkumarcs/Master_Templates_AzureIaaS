...
<#
.SYNOPSIS
    This runbook will scale down an ARM VM in response to an Azure alert.

.DESCRIPTION
    This runbook will scale down an ARM VM in response to an Azure alert.
    https://docs.microsoft.com/en-us/azure/automation/automation-create-alert-triggered-runbook
    Input is alert data with information needed to identify which VM to scale down.
    ...
    if (($status -eq "Activated") -or ($status -eq "Fired"))
    {
        ...
        if ($ResourceType -eq "Microsoft.Compute/virtualMachines")
        {
            ...
            if($newVMSize -eq $noResize)
            {
                ...
                Write-Verbose "The current VM size can''t be scaled down. You''ll need to recreate the specified Virtual Machine with your requested size." -Verbose
            }
            else
            {
                ...
                Write-Verbose ("The VM size has been scaled down from $currentVMSize to " + $updatedVm.HardwareProfile.vmSize) -Verbose
            }
        }
        else {
